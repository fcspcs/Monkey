...
{
    private const string ServiceName = "MonkeySrv";
    private const string DisplayName = "Monkey Zeitkontrolle";
    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            // Vorgaengerversion hiess TimeGuard. Deren Dienst laeuft eigenstaendig
            // weiter und wuerde weiterhin abmelden - er muss zuerst mit dem alten
            // Installer und dem alten Master-Passwort entfernt werden.
            if (LegacyServiceInstalled())
            {
                Banner();
                Error("Auf diesem Rechner ist noch die Vorgaengerversion \"TimeGuard\" installiert.");
                Console.WriteLine();
                Console.WriteLine("  Bitte zuerst TimeGuard entfernen - mit der alten TimeGuardSetup.exe,");
                Console.WriteLine("  Punkt [2] Entfernen und dem bisherigen Master-Passwort.");
                Console.WriteLine("  Danach diesen Installer erneut starten.");
                Console.WriteLine();
                Console.WriteLine("  Grund: Der alte Dienst laeuft sonst parallel weiter und meldet dich");
                Console.WriteLine("  weiterhin ab. Monkey kann ihn nicht ohne das alte Passwort abbauen.");
                return Pause(2);
            }
            ...
            Console.WriteLine(installed
                ? "Monkey ist auf diesem Rechner installiert."
                : "Monkey ist auf diesem Rechner noch nicht installiert.");
            Console.WriteLine();
    ...
    private static int Install(bool alreadyInstalled)
    {
        ...
        {
            Console.WriteLine();
            Warn("Es ist bereits eine Monkey-Installation vorhanden (moeglicherweise defekt).");
    ...
    private static int Uninstall(bool installed)
    {
        ...
        foreach (var p in Process.GetProcessesByName("MonkeyAgent"))
            TryKill(p);
    ...
    /// <summary>Ist der Dienst der Vorgaengerversion "TimeGuard" noch registriert?</summary>
    private static bool LegacyServiceInstalled()
    {
        try
        {
            using var key = Microsoft.Win32.Registry.LocalMachine.OpenSubKey(
                @"SYSTEM\CurrentControlSet\Services\TimeGuardSrv");
    ...
    private static void Banner()
    {
        Console.WriteLine();
        Console.WriteLine("  ============================================");
        Console.WriteLine("   Monkey - Zeitkontrolle einrichten");
        Console.WriteLine("  ============================================");
        Console.WriteLine();
    }

...
/// <summary>
/// Zentrale Ablageorte. Alles Zustandsbehaftete liegt unter %ProgramData%\Monkey,
/// damit der Ordner per ACL gegen den angemeldeten Benutzer abgedichtet werden kann.
/// </summary>
public static class Paths
{
    public const string DefaultPipeName = "Monkey.v1";
    public const string ServiceName = "MonkeySrv";

    /// <summary>Sitzungslokal: pro angemeldetem Benutzer laeuft genau ein Agent.</summary>
    public const string MutexName = @"Local\Monkey.Agent";

    private static string _dataDir = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "Monkey");
